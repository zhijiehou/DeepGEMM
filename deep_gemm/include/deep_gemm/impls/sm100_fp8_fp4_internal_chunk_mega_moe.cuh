...
template <
    ...
    uint32_t kNumChunks, uint32_t kChunkTokensPerRank,
...
sm100_fp8_fp4_internal_chunk_mega_moe_impl(void* y,
                            int* cumulative_local_expert_recv_stats,
                            const uint32_t num_tokens,
                            const uint32_t chunk_tokens,
                            const void* __restrict__ x_user,
                            const void* __restrict__ x_sf_user,
                            const void* __restrict__ topk_idx_user,
                            const void* __restrict__ topk_weights_user,
    ...
    // Workspaces
    // iter8: layout sizing uses kChunkTokensPerRank (per-chunk slot size), not full T_max.
    // For num_chunks=1 these are equal (no-op); for num_chunks>1 the symm buffer is
    // chunk-sized so layout offsets must also be chunk-sized to avoid out-of-bounds.
    const auto workspace = layout::Workspace(
        sym_buffer.get_base_ptr(), kNumRanks, kNumExperts, kChunkTokensPerRank, kNumTopk);
    ...
    // Registered inputs
    // iter8: per-chunk slot sizing — see Workspace comment above.
    // iter8: per-chunk slot sizing — see Workspace comment above.
    const auto input_token_buffer = layout::Buffer(
        fp8_token_layout, 1, kChunkTokensPerRank,
        workspace.get_end_ptr());
    const auto input_sf_buffer = layout::Buffer(
        fp8_sf_layout, 1, kChunkTokensPerRank,
        input_token_buffer.get_end_ptr());
    const auto input_topk_idx_buffer = layout::Buffer(
        input_topk_idx_layout, 1, kChunkTokensPerRank,
        input_sf_buffer.get_end_ptr());
    const auto input_topk_weights_buffer = layout::Buffer(
        input_topk_weights_layout, 1, kChunkTokensPerRank,
    ...
    // Combine inputs
    // iter8: combine slot also chunk-sized (each chunk's combine outputs land in same slot,
    // then dispatch warps DMA them out to user y at chunk_token_start offset — to be added in iter9).
    const auto combine_token_buffer = layout::Buffer(
        bf16_token_layout, kNumTopk, kChunkTokensPerRank,
    ...
    constexpr uint32_t kChunkBoundaryBarrierIdx = 15;  // all-threads chunk sync
    ...
    if (warp_idx < kNumDispatchWarps) {
        ...
        // iter9: Lift pull mbarrier phase out of chunk loop so it correctly tracks
        // hardware state across chunk iterations (each chunk fires `chunk_actual` times).
        uint32_t pull_mbarrier_phase = 0;

        // iter9: chunk loop wrapper — staging + pull + workspace clean per chunk
        #pragma unroll 1
        for (uint32_t chunk_idx = 0; chunk_idx < kNumChunks; ++ chunk_idx) {
            const uint32_t chunk_token_start = chunk_idx * kChunkTokensPerRank;
            const uint32_t chunk_actual = (chunk_token_start + kChunkTokensPerRank <= num_tokens)
                ? kChunkTokensPerRank : (num_tokens - chunk_token_start);

            // For chunks beyond the first, clear smem_expert_count (used as atomicAdd_block accumulator).
            if (chunk_idx > 0) {
                #pragma unroll
        ...
        // ===========================================================================
        // iter7: In-kernel input staging (grid-partitioned)
        //
        // iter9: now per-chunk — staging copies the slice
        // [chunk_token_start, chunk_token_start + chunk_actual) into the
        // chunk-sized symm buffer slots, refreshing them each chunk.
        // ===========================================================================
        {
            constexpr uint32_t kCopyBytesPerThread = 16;  // uint4
            const uint32_t global_dispatch_idx = sm_idx * kNumDispatchThreads + thread_idx;
            constexpr uint32_t kGlobalDispatchThreads = kNumSMs * kNumDispatchThreads;

            const auto stage_copy = [&](const void* src_v, void* dst_v, const uint32_t total_bytes) {
                const auto src = static_cast<const uint8_t*>(src_v);
                const auto dst = static_cast<uint8_t*>(dst_v);
                #pragma unroll 1
                for (uint32_t off = global_dispatch_idx * kCopyBytesPerThread;
                     off + kCopyBytesPerThread <= total_bytes;
                     off += kGlobalDispatchThreads * kCopyBytesPerThread) {
                    *reinterpret_cast<uint4*>(dst + off) = __ldg(reinterpret_cast<const uint4*>(src + off));
                }
            };

            const uint8_t* x_user_chunk = static_cast<const uint8_t*>(x_user) + chunk_token_start * kHidden;
            const uint8_t* x_sf_user_chunk = static_cast<const uint8_t*>(x_sf_user) + chunk_token_start * (kHidden / 32);
            const uint8_t* topk_idx_user_chunk = static_cast<const uint8_t*>(topk_idx_user) + chunk_token_start * kNumTopk * sizeof(int64_t);
            const uint8_t* topk_weights_user_chunk = static_cast<const uint8_t*>(topk_weights_user) + chunk_token_start * kNumTopk * sizeof(float);

            stage_copy(x_user_chunk, input_token_buffer.get_base_ptr(),
                       chunk_actual * kHidden);
            stage_copy(x_sf_user_chunk, input_sf_buffer.get_base_ptr(),
                       chunk_actual * (kHidden / 32));
            stage_copy(topk_idx_user_chunk, input_topk_idx_buffer.get_base_ptr(),
                       chunk_actual * kNumTopk * sizeof(int64_t));
            stage_copy(topk_weights_user_chunk, input_topk_weights_buffer.get_base_ptr(),
                       chunk_actual * kNumTopk * sizeof(float));

            // Flush staging writes from L1 to L2 for cross-SM visibility
            // All SMs must finish staging before any dispatch warp reads inputs
            comm::cluster_grid_sync<kNumSMs, 2, kDispatchGridSyncIndex>(
                workspace, sm_idx, thread_idx, is_leader_cta,
                [=]() { ptx::sync_aligned(kNumDispatchThreads, kDispatchBarrierIdx); }
            );
        }
        ...
        const auto read_topk_idx = [&](const auto& process) {
            ...
            // iter9: bound is chunk_actual (chunk-local indexing); input_topk_idx_buffer
            // already holds the chunk-local slice from staging.
            #pragma unroll
            for (uint32_t i = (sm_idx * kNumDispatchWarps + warp_idx) * kNumTokensPerWarp;
                ...
                int expert_idx = -1;
                if (i + (lane_idx / kNumTopk) < chunk_actual and lane_idx < kNumActivateLanes) {
        ...
        comm::cluster_nvlink_barrier<kNumRanks, kNumSMs, 2, kNumDispatchThreads,
                             kDispatchGridSyncIndex, kBeforeDispatchPullBarrierTag>(
            workspace, sym_buffer, sm_idx, thread_idx, is_leader_cta,
            [=]() { ptx::sync_aligned(kNumDispatchThreads, kDispatchBarrierIdx); },
        ...
        // iter9: pull_mbarrier_phase is lifted out of the chunk loop (declared above).
        ...
        scheduler.fetch_expert_recv_count(chunk_idx);
        ...
        if (sm_idx == 0) {
            ...
            for (uint32_t i = sm_idx - 1; i < kNumExpertsPerRank; i += kNumSMs - 1) {
                // Compute per-chunk recv token count from per-rank recv_count
                // (expert_recv_count_sum accumulates across chunks; per-rank counts are zeroed each chunk)
                ...
                if (warp_idx == 0) {
                    // Zero recv_count_sum only on last chunk (for next kernel call)
                ...
                // Clean per-rank token count (must always run for next kernel invocation)
        ...
        // Ensure all cleanup writes are globally visible before cross-SM sync

        // Wait for all ranks to finish cleaning
        // iter9: the "end of kernel" sync flag stays false because the next chunk's
        // dispatch staging issues its own grid_sync after the user-input copy.
        comm::cluster_nvlink_barrier<kNumRanks, kNumSMs, 2, kNumDispatchThreads,
        ...
        // Chunk boundary sync: all warp groups converge before next chunk
        if (chunk_idx < kNumChunks - 1) ptx::sync_unaligned(kNumThreads, kChunkBoundaryBarrierIdx);
        }  // iter9: end chunk_idx loop (dispatch warps)
    } else if (warp_idx == kNumDispatchWarps) {
        // Adjust registers
        cutlass::arch::warpgroup_reg_dealloc<kNumNonEpilogueRegisters>();

        // iter9: GEMM A-load warp wraps each chunk's scheduler iteration
        #pragma unroll 1
        for (uint32_t chunk_idx = 0; chunk_idx < kNumChunks; ++ chunk_idx) {

        // GEMM TMA load warp for tokens with SFA
        scheduler.for_each_block(chunk_idx, [&](const sched::BlockPhase& block_phase,
        ...
        if (chunk_idx < kNumChunks - 1) ptx::sync_unaligned(kNumThreads, kChunkBoundaryBarrierIdx);
        }  // iter9: end chunk_idx loop (GEMM A-load warp)
    } else if (warp_idx == kNumDispatchWarps + 1) {
        // Adjust registers
        cutlass::arch::warpgroup_reg_dealloc<kNumNonEpilogueRegisters>();

        // iter9: GEMM B-load warp wraps each chunk's scheduler iteration
        #pragma unroll 1
        for (uint32_t chunk_idx = 0; chunk_idx < kNumChunks; ++ chunk_idx) {
        ...
        if (chunk_idx < kNumChunks - 1) ptx::sync_unaligned(kNumThreads, kChunkBoundaryBarrierIdx);
        }  // iter9: end chunk_idx loop (GEMM B-load warp)
        ...
        if (is_leader_cta) {
            ...
            // iter9: current_iter_idx lifted out of chunk loop so accum_stage_idx /
            // accum_phase track the tmem mbarrier hardware state continuously across chunks.
        ...
        if (chunk_idx < kNumChunks - 1) ptx::sync_unaligned(kNumThreads, kChunkBoundaryBarrierIdx);
            }  // iter9: end chunk_idx loop (MMA warp)

            // To safely deconstruct barriers, we need another round of waits
            // iter9: deconstruct stays outside the chunk loop — only the final
            // chunk's last accum stage needs to be drained before the kernel exits.
            ...
            // Non-leader CTA MMA warp: no MMA work but must participate in chunk boundary sync
            #pragma unroll 1
            for (uint32_t chunk_idx = 0; chunk_idx < kNumChunks; ++ chunk_idx) {
                if (chunk_idx < kNumChunks - 1) ptx::sync_unaligned(kNumThreads, kChunkBoundaryBarrierIdx);
            }
        ...
        // This warp has no work but must participate in chunk boundary sync
        #pragma unroll 1
        for (uint32_t chunk_idx = 0; chunk_idx < kNumChunks; ++ chunk_idx) {
            if (chunk_idx < kNumChunks - 1) ptx::sync_unaligned(kNumThreads, kChunkBoundaryBarrierIdx);
        }
        ...
        // iter9: lift mbarrier phase rotors OUT of the chunk loop so they correctly
        // track hardware state across chunk iterations. Both `current_iter_idx`
        // (drives accum_stage_idx / accum_phase for tmem barriers) and the combine
        // load mbarrier rotor (`combine_phase`, `load_stage_idx`) are persistent
        // hardware state — resetting them per chunk would race the mbarriers.
        uint32_t current_iter_idx = 0;
        uint32_t combine_phase = 0;
        uint32_t load_stage_idx = 0;

        // iter9: chunk loop wrapper for epilogue+combine warp
        #pragma unroll 1
        for (uint32_t chunk_idx = 0; chunk_idx < kNumChunks; ++ chunk_idx) {
            const uint32_t chunk_token_start = chunk_idx * kChunkTokensPerRank;
            const uint32_t chunk_actual = (chunk_token_start + kChunkTokensPerRank <= num_tokens)
        ...
        // iter9: tensor memory dealloc moved OUT of the chunk loop (one-time at kernel exit).

        // NVLink barrier (grid sync + cross-rank signal + grid sync): ~4 us
        comm::cluster_nvlink_barrier<kNumRanks, kNumSMs, 2, kNumEpilogueThreads,
                             kEpilogueGridSyncIndex, kBeforeCombineReduceBarrierTag>(
            workspace, sym_buffer, sm_idx, epilogue_thread_idx, is_leader_cta,
            [&]() { ptx::sync_aligned(kNumEpilogueThreads, kEpilogueFullBarrierIdx); }
        );
        ...
        DG_STATIC_ASSERT(kHidden % kNumCombineChunks == 0, "Hidden must be divisible by number of chunks");
        DG_STATIC_ASSERT(kNumCombineChunkSlots * kNumEpilogueWarps * kNumHiddenBytes / kNumCombineChunks <= SMEM_BEFORE_BARRIER_SIZE, "Hidden is too large");
        ...
        // Iterate over all tokens (chunk-local: input_topk_idx_buffer holds the staged
        // chunk slice, so we index by [0, chunk_actual). The y-write below adds
        // `chunk_token_start * kNumHiddenBytes` to land at the global token row.)
        // iter9: combine_phase / load_stage_idx are lifted to outside the chunk loop
        // (declared above) — they track the combine_load_barriers' hardware phase
        // and must persist across chunks.
        for (uint32_t token_idx = sm_idx * kNumEpilogueWarps + epilogue_warp_idx;
             token_idx < chunk_actual;
             token_idx += kNumSMs * kNumEpilogueWarps) {
            // Read top-k slot indices: each lane reads one slot, then broadcast via exchange
            DG_STATIC_ASSERT(kNumTopk <= 32, "Invalid number of topk");
            const int stored_topk_slot_idx = lane_idx < kNumTopk ?
                static_cast<int>(*(input_topk_idx_buffer.get_base_ptr<int64_t>() + token_idx * kNumTopk + lane_idx)) : -1;
            ...
            for (uint32_t chunk = 0; chunk < kNumCombineChunks; ++ chunk) {
                ...
                // iter9: y-write adds `chunk_token_start * kNumHiddenBytes` so chunk-local
                // `token_idx` lands at the correct global token row.
                if (cute::elect_one_sync()) {
                    cute::tma_store_fence();
                    ptx::tma_store_1d(
                        math::advance_ptr(y, (static_cast<uint64_t>(chunk_token_start) + token_idx) * kNumHiddenBytes + chunk_byte_offset),
        ...
        if (chunk_idx < kNumChunks - 1) ptx::sync_unaligned(kNumThreads, kChunkBoundaryBarrierIdx);
        }  // iter9: end chunk_idx loop (epilogue+combine warp)

        // iter9: tensor memory dealloc, called once after all chunks complete.
